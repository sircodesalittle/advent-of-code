...
fn process_map_line(map_line: &str, current: &mut SpecialMap) {
    ...
    let range_length = map_line_split[2].parse::<u128>().unwrap() - 1;
    let new_band = Band {
        source_start: source,
...
fn main() {
    ...
    let mut seeds_line = String::from("");
            ...
            if let Ok(ready_line) = line {
                if ready_line != "" {
                    if ready_line.contains("seeds: ") {
                        seeds_line = String::from(ready_line);
    ...
    let mut seed_range: u128 = 0;
    for val in seeds_str.split(" ") {
        let val_int = val.parse::<u128>().unwrap();
        if !in_seed {
            seed_start = val_int;
            in_seed = true;
        } else {
            seed_range = val_int - 1;
            let seed_end = seed_start + seed_range;
            in_seed = false;
            dbg!(seed_start..seed_end);
            for seed in seed_start..seed_end {
                ...
                if location < lowest_location {
                    ...
                    println!("Updating lowest location to: {}", lowest_location);
                }
            }
        }
    }
